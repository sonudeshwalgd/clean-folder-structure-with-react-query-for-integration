...
export default function AddBioPopup({
  toggleVisibility,
  payloadId,
  refetch,
}: OTPPropsType) {
  const bio = useRef<any>();
  const statement = useRef<any>();

  const { mutate: patchBioRequest, data: patchNews } = useMutation(
    ["patchDirectorProjectViewBio"],
    patchDirectorProjectViewBio,
    {
      onSuccess: (res) => {
  ...
  const patchHandler = () => {
    var payload = {
      ...
      statement: statement.current.value,
    };
    console.log(payload);
    patchBioRequest(payload);
  };
